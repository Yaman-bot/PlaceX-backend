...
const getUsers = async (req, res, next) => {
    ...
    let users;
    try {
        //Finding by email & name,therefore removing password from the object
...
const signup = async (req, res, next) => {
    ...
    let hashedPassword;
    try {
        hashedPassword = await bycrpyt.hash(password, 12)
    } catch (err) {
...
const login = async (req, res, next) => {
    ...
    let isValidPassword;
    try {
        isValidPassword = await bycrpyt.compare(password, existingUser.password)
    } catch (err) {
